...
, nodejs ? pkgs.nodejs-14_x
  # The specific version of `spago` to use. Will be used internally as well as
  # provided in the `devShells` generated by `spago.nix`
  #
  # (NOTE: There is no guarantee that older versions of `spago` will work correctly
  # for all outputs)
, spago ? pkgs.purescriptPackages.spago
, shell ? { }
...
let
  inherit (pkgs) lib purescriptPackages;
  ...
  # suitable for `purs` versions from `difficult-purescript-nix`
  compilerVersion =
  ...
  compiler = purescriptPackages."purs-${compilerVersion}";
  ...
  lsDeps = import
    (
      pkgs.runCommandLocal "${name}-ls-deps"
        {
          buildInputs = [ spago pkgs.jq fakePackagesDhall ];
        }
        ''
          ${prepareFakeSpagoEnv}
  ...
  fakePsa =
    ...
    pkgs.writeShellApplication {
      name = "psa";
      runtimeInputs = [ purescriptPackages.psa ];
      text = ''
        psa ${psaArgs} "$@"
      '';
    };
  ...
  bundle =
    ...
    , ...
      ...
      pkgs.runCommand ''${args.name or "${name}-bundle-${type}"}''
        {
          nativeBuildInputs =
            [ output compiler spago pkgs.git ]
  ...
  docsDeps = { format ? "html" }:
    ...
    pkgs.runCommand
      "${name}-deps-docs"
      {
        nativeBuildInputs = [ compiler spago pkgs.git nodejs ];
      }
      ''
        ${prepareFakeSpagoEnv}
...
rec {
  ...
  tools = {
    inherit (purescriptPackages) spago;
    purs = compiler;
  } // builtins.listToAttrs
    (builtins.map (n: lib.nameValuePair n purescriptPackages.${n}) (shell.tools or [ ]))
  ;
}
