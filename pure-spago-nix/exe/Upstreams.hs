...
toDrvs =
  -- TODO replace this with `NixExpr` once function application is defined
  fmap Prettyprinter.sep . \case
    Dhall.RecordLit pkgs -> for (Dhall.Map.toList pkgs) $
      \(name, pkg) -> case pkg.recordFieldValue of
        Dhall.RecordLit r -> do
          repo <- extractText =<< lookupOrThrow "repo" r
          rev <- extractText =<< lookupOrThrow "version" r
          sha256 <- extractText =<< lookupOrThrow "sha256" r
          pure $
            Prettyprinter.sep
              [ quoteText name <+> " = pkgs.stdenv.mkDerivation "
              , Prettyprinter.nest 2 $
                  Prettyprinter.sep
                    [ "{"
                    , "version"
                        ...
                        <+> quoteText rev
                        <> Prettyprinter.semi
                    , "phases" <+> Prettyprinter.equals
                        <+> quoteText "installPhase"
                        <> Prettyprinter.semi
                    , "installPhase" <+> Prettyprinter.equals
                        <+> quoteText "ln -s $src $out"
                        <> Prettyprinter.semi
                    , "src" <+> Prettyprinter.equals <+> "pkgs.fetchGit "
                    , Prettyprinter.nest 2 $
                        Prettyprinter.sep
                          [ "{"
                          , "url"
    ...
    expr -> throwOnExpr expr
  where
    quoteText :: Text -> Prettyprinter.Doc ann
    quoteText = Prettyprinter.dquotes . Prettyprinter.pretty
...
addSha256 pkg r = do
  repo <- extractText =<< lookupOrThrow "repo" r
  ref <- extractText =<< lookupOrThrow "version" r
  prefetched <-
    checkExitCode
      =<< Process.Typed.readProcessStdout
        ( Process.Typed.proc
            "nix-prefetch-git"
            ["--quiet", repo ^. unpacked, "--rev", ref ^. unpacked]
        )
