...
- [[#top-level-attributes][Top-level attributes]]
  ...
  - [[#spago-nixutils][~spago-nix.utils.*~]]
    - [[#apps][~apps~]]
...
** ~spago-nix.utils.*~
This contains several utilities for working with derivations related to Purescript. As most of these can be applied outside of the context of ~spagoProject~, they are not returned from that function unlike the [[#library-functions][other functions]] documented below
*** ~apps~
**** ~fromNodeApp~
